...
class TargetGPSNode(Node):
    ...
    def try_compute(self):
        ...
        total_bearing_deg = math.degrees(total_bearing_rad)

        print(f"total_bearing = {total_bearing_deg:.2f}")

        theta = math.pi/2 - total_bearing_rad

        E =  E_cam * math.cos(theta) - N_cam * math.sin(theta)
        N =  E_cam * math.sin(theta) + N_cam * math.cos(theta)
