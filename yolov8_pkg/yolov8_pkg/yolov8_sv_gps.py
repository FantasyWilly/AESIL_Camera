...
class YoloRtspRosNode(Node):
    def __init__(self):
        ...
        # **宣告並讀取參數**
        self.declare_parameter('model_path', '/home/fantasywilly/weight/Car_Model.engine')
        ...
        self.declare_parameter('rtsp_server_url', 'rtsp://192.168.2.231:8554/live/stream')
        ...
        # **用來存 tracker_id → (lat, lon) 的對應關係**
        self.current_gps_dict = {}
    ...
    def gps_callback(self, msg):
        # **用 frame_id 當 key，把對應的經緯度記下來**
        tracker_id = msg.header.frame_id
        with self.gps_lock:
            self.current_gps_dict[tracker_id] = (msg.latitude, msg.longitude)
        self.get_logger().debug(
            f"收到目標 {tracker_id} GPS：緯度 {msg.latitude:.7f}, 經度 {msg.longitude:.7f}"
        )
            ...
            try:
                ...
                with self.gps_lock:
                    for cls, tracker_id in zip(detections.class_id, detections.tracker_id):
                        # **嘗試從字典取出該 tracker_id 的經緯度，預設 (0,0)**
                        lat, lon = self.current_gps_dict.get(str(tracker_id), (0.0, 0.0))
                        labels.append(
                            f"#{tracker_id} {self.model.names[int(cls)]} - [{lat:.7f}, {lon:.7f}]"
                        )
    ...
    def publish_detections(self, detections):
        """經過 SuperVision 解析帶 - [tracker_id] 的 Detections 發布 BoundingBox 與 CenterBox 訊息"""
        for box, cls, conf, tracker_id in zip(detections.xyxy, detections.class_id, detections.confidence, detections.tracker_id):
            ...
            center_msg.tracker_id = int(tracker_id)
